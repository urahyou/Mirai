...
test('agent gateway executes automatic capabilities with a minimal snapshot and audit', async () => {
  const received = [];
  const audit = createAgentAudit({ now: () => 1000 });
  const gateway = createAgentGateway({
    audit, now: () => 1000,
    providers: { pi: { execute: async (task) => { received.push(task); task.snapshot.weather.summary = 'mutated'; return { summary: '完成' }; } } },
  });
  ...
  assert.deepEqual(audit.list().map((entry) => entry.type), [
    'agent.execution.completed', 'agent.execution.started', 'agent.task.proposed',
  ]);
  assert.equal(JSON.stringify(audit.list()).includes('晴，28°C'), false, 'audit must not copy task snapshot content');
});
...
test('agent gateway holds confirmation capabilities until one approval', async () => {
  let executions = 0;
  const audit = createAgentAudit();
  const gateway = createAgentGateway({
    audit,
    providers: { pi: { execute: async () => { executions += 1; return { summary: 'drafted' }; } } },
  });
...
test('agent gateway rejects pending tasks without provider execution', async () => {
  let executions = 0;
  const gateway = createAgentGateway({ providers: { pi: { execute: async () => { executions += 1; } } } });
...
test('agent gateway rejects provider result escalation and redacts raw failures', async () => {
  const audit = createAgentAudit();
  const gateway = createAgentGateway({
    audit,
    providers: { pi: { execute: async () => ({
  ...
  assert.equal(result.error, 'Provider 执行失败');
  assert.equal(JSON.stringify(result).includes('whoami'), false);
  assert.equal(JSON.stringify(audit.list()).includes('whoami'), false);
});

test('agent gateway contains provider failures and missing providers', async () => {
  const failed = createAgentGateway({ providers: { pi: { execute: async () => { throw new Error('provider crashed'); } } } });
  ...
  assert.equal(crashed.error, 'Provider 执行失败');
  assert.equal(JSON.stringify(crashed).includes('provider crashed'), false);
  const missing = createAgentGateway({});
  const result = await missing.request({ capability: 'context.time', objective: 'time', snapshot: { now: 'now' } });
  assert.equal(result.state, 'failed');
