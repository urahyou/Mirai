...
- 主进程按职责拆分，以依赖注入创建（main.js 只做排序与注入）：
  ...
  - **Agent 安全内核**：`src/contracts/agent.js` 是能力/风险单源，`src/services/agent-gateway.js` 只向 Provider 下发最小快照并强制审批状态机，`agent-audit.js` 只记录有界元数据。确认类任务必须按“Provider 先提案 → 用户看到精确参数后审批 → 主进程白名单 action handler 单次执行”，任何 Provider 都不得绕过 Gateway 或直接写领域状态。Pi Provider 在 `src/services/pi-execution-provider.js`，每任务独立 RPC 子进程且只加载 `scripts/agent/mirai-proposal.ts`；不得恢复内置 bash/edit/write 或继承项目密钥环境变量。当前唯一 action handler 是 `src/services/agent-actions.js` 的固定草稿目录写入，新增动作必须先加入能力契约、参数校验、审批和审计测试。
...
- **上下文压缩阈值**：当对话上下文占用达到约 80% 时自动压缩/紧凑摘要，不要等到接近上限。机制：① pi 自动压缩钩子——`~/.pi/agent/settings.json` 的 `compaction` 已按默认模型窗口设 `reserveTokens≈20%`（触发点在 80%占用）；换模型需重算 `reserveTokens=round(0.2×新模型窗口)`。② 主动觉察——长会话中自行判断并执行 `.agents/skills/context-compression/SKILL.md`（写续接摘要、归档详细、可 `/compact`），换模型感知无关兜底。原则：宁早勿晚，压缩只为保留连续性。
