# AGENTS.md

Electron 桌宠「小未来」——类似伪春菜 (Ukagaka) 的透明悬浮桌面字符，对话由本地大模型驱动。

## 常用命令

- 启动：`npm start`
- 开发调试（打印渲染进程 console、开 DevTools）：`npm run dev`
- 无 lint / test / typecheck 脚本，验证方式 = 启动后观察 `/tmp/pet_*.log` 是否有报错。

## 架构（非直觉，务必先读）

- 入口：`src/main/main.js`（主进程：窗口、IPC、对话调度、provider 状态）
- 渲染进程 `src/renderer/`：角色 UI、气泡、拖拽、**右键 HTML 菜单**（托盘已移除，勿再加 Tray）
- 安全桥接 `src/main/preload.js`：所有 IPC 经 `desktopPet.*` 暴露，渲染进程不直接 require。

对话调度（`main.js` 的 `sendChat`）：
1. 先识别「我叫X/我的名字是X」主人称呼 → 写入 owner.json
2. `parseMemoryIntent` 识别记忆指令（记住/忘记/回查）→ 记忆服务处理
3. 其余全部走 `src/engine/generic.js` 调本地大模型（按 provider 优先级自动回退）
4. 兜底回复

> 本地规则关键词答话已移除（无 `dialogueMode` 概念），对话一律走大模型。

## 配置（不要硬编码进代码）

- `src/core/llm-providers.json`：**LLM provider 唯一配置来源**。`activeProvider` 决定用哪个。改模型/地址都改这里，勿改 main.js。
  - 当前 DeepSeek 本地 vLLM：`http://192.168.99.99:8000/v1`，模型 `deepseek-v4-flash`（vLLM 常用 8000 端口）。
  - `isAvailable()` 探测 `/models` 端点；`apiKey` 为 `none`/`EMPTY` 时不加 Authorization 头。
- `src/core/personality.json`：角色出厂人格（只读默认）。`systemPrompt` 中的 `{personality}` 会被替换成整个 personality 对象注入给大模型。
  - 用户编辑的人格覆盖存 userData `personality-runtime.json`（深合并到出厂之上）；主人在「关于主人」页面编辑的称呼存 userData `owner.json`。

## 关键坑（容易踩）

- **角色图**：`assets/character/` 下 `idle.png`/`happy.png`/`sad.png`（1254×1254），按情感切换，命名勿乱改。相对路径是 `../../assets/...`（renderer 在 `src/renderer`，asset 在根目录，两级向上）。
- **macOS Gatekeeper**：旧版 Electron（如 31）公证被吊销，运行时报 `SIGKILL`/「包含恶意软件」。当前 Electron 43.3.0 正常。若再遇到该报错，是二进制下载不完整/公证吊销，不是代码问题。
- **Electron 二进制**：当前 Node 是 v20（brew `node@20`），而新版 Electron 下载工具 `@electron/get@5` 需 Node≥22。若 `npm start` 报 `ENOENT`/闪退，说明 `node_modules/electron/dist` 二进制缺失，需手动补（`path.txt` 内容为 `Electron.app/Contents/MacOS/Electron`）。
