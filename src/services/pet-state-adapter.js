...
module.exports = function createPetStateAdapter({ pythonBackend, fallback }) {
  ...
  function applyEvent(type) {
    ...
    const now = Date.now();
    void pythonBackend.request('pet.apply_event', { eventType: type, now }).then((result) => {
      cache = result.state;
    ...
    }).catch(() => { cache = fallback.applyEvent(type); });
    return getState();
  }
  ...
  return { init, getState, describe, applyEvent, seedFromLegacy };
};
